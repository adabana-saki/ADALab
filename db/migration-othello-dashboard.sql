-- Migration: Add othello to user_game_stats and user_achievements CHECK constraints
-- Run: npx wrangler d1 execute adalab_db --remote --file=./db/migration-othello-dashboard.sql

-- SQLite doesn't support ALTER CHECK constraint, so we recreate the tables

-- Step 1: Recreate user_game_stats with othello support
CREATE TABLE IF NOT EXISTS user_game_stats_new (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id INTEGER NOT NULL,
  game_type TEXT NOT NULL CHECK(game_type IN ('tetris', '2048', 'snake', 'typing', 'minesweeper', 'othello')),
...
-- Step 2: Recreate user_achievements with othello support
