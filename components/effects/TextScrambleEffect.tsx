...
export function TextScrambleEffect() {
  ...
  useEffect(() => {
    ...
    elements.forEach((el) => {
      ...
      scrambleRefs.current.set(htmlEl, scrambler);
    ...
    const autoScramble = setInterval(() => {
      ...
      if (headings.length > 0) {
        ...
        if (!scrambleRefs.current.has(randomHeading)) {
          const scrambler = new TextScrambler(randomHeading);
          scrambleRefs.current.set(randomHeading, scrambler);
          scrambler.setText(originalText);
        } else {
          scrambleRefs.current.get(randomHeading)?.setText(originalText);
        }
      }
